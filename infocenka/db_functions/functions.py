...
def get_jsons_from_our_db(
        conn_params,
        query ='select activity_json from orion.export_for_valuation',
        estate_type = None):
    ...
    try:
        with SSHTunnelForwarder(
            ...
            conn = psycopg2.connect(
                        ...
                        password=conn_params['password'],
                        port = tunnel.local_bind_port)
            postgres_insert_query = f"""INSERT INTO valuation.inform_ocenka_all(activity_id,\
                modified, preds_knn, preds_rfr, preds_cbr, credibility, expos_weeks)\
...
def add_to_our_inform_ocenka_all(conn_params, data):
    ...
    try:
        conn = psycopg2.connect(
            ...
            port = conn_params['port'])
        print('ok')
        postgres_insert_query = f"""INSERT INTO orion.export_for_valuation
...
def norm_for_insert(x):
    ...
    x = tuple(x)
    return x
def copy_data_to_our_bd(conn_params, tunnel_params_from, conn_params_from, data=None):
    """загружаем из БД И.Л. и добавим в нашу базу"""
    if data:
        pass
    else:
        print('load from armr_db...')
        q = """select activity_id, modified, activity_json from valuation.export_for_valuation"""
        data = get_jsons(tunnel_params_from, conn_params_from, query=q)
        data = list(map(norm_for_insert, data))
    s=f"""INSERT INTO orion.export_for_valuation (activity_id,\
    ...
    with psycopg2.connect(
        ...
        for i in range(len(data)):
            try:
                ...
                fail+=1
        print(f'УСПЕШНО ДОБАВИЛИ {len(data)-fail}')
        print(f'не удалось дабавить {fail} {data[i]}')
        conn.commit()
    # conn.close()
